...
import { AtTabs, AtTabsPane, AtActivityIndicator, AtLoadMore, AtButton } from 'taro-ui';
import { getFocusData, getRcmdData, getHotData } from '@/api/index';
import SearchInput from '@/components/searchInput/index';
import { showSuccess } from '@/utils/index';
import editPng from '@/assets/images/edit.png'
import toRecommendPng from '@/assets/images/to-recommend.png'
import './index.scss';
...
export default class Index extends Component {
    ...
    setTimeout(() => {
      getRcmdData()
        .then(res => {
          ...
          // });
          this.setState((state) => {
            const recList = state.recList.concat(res.list);
            const status = recList.length>=30 ? 'noMore':'more'
            return {
              recList,
              status
            }
  ...
  // 获取更多热榜列表
  getMoreHotList = () => {
    if (this.state.hotStatus === 'noMore') {
      return;
    }
    ...
    setTimeout(() => {
      getHotData()
        .then(res => {
          ...
          // });
          this.setState(state => {
            const hotList = state.hotList.concat(res.list);
            const hotStatus = hotList.length>=30 ? 'noMore':'more'
            return {
              hotList,
              hotStatus
            }
  ...
  goTitleDetail = (event, {id, title}) => {
    Taro.navigateTo({
      url:
        '/pages/titleDetail/titleDetail?id='+id +'&title=' + encodeURI(title)
    });
  };
  goContentDetail = (event, item) => {
    Taro.navigateTo({
      url:
            ...
            <View className={'tab-content '}>
              ...
              {focusList.length == 0 && (
                <View>
                  ...
                  <View className="to-recommend-tip" onClick={() => this.setActive(1)}>去【推荐】看看吧</View>
                </View>
              )}
              {/* TODO: 以下注释暂时不能去掉 */}
              {/* {focusList.length != 0 && (
                    ...
                    return (
                      <View className="tab-content-recommend" key={index}>
                        ...
                        <View
                          ...
                          onClick={e => this.goTitleDetail(e, item)}>
                          {item.title}
                        </View>
                        <View
